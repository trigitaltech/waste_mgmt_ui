...
import {
  payrolldriver,deletepayrolldriver,lgus,type
} from '../../../../services/auth'
import NProgress from 'nprogress/nprogress'
import Createstaging from '../account/Createstaging.vue'
export default {
  page: {
    title: 'Payroll Driver',
 ...
 async deleteReq(data) {
     ...
     try{
        ...
        const result = await deletepayrolldriver(data.item.id)
        if (result) {
          this.$swal({
            group: 'alert',
            type: 'success',
            text: `You Deleted Payroll Driver  Successfully`,
            duration: 5000,
          })
         this.refresh()
        }
    ...
    <div class="animated fadeIn">
      ...
      ><b-row>
        ...
        <b-col >
            ...
            text="Create Tenant"
            @click="$router.push({ path: '/CreatePayrollDriver' })"
          >Create Payroll Driver</b-button>
        </b-col>
        </b-row>
        <div class="mt-3">
          <b-table
          ...
          >
           <template v-slot:cell(actions)="data">
            <router-link :to="{ name: 'Viewpayrolldriver', params: data.item }">
                <span class="mr-3" >
                 <i class="fa fa-eye edit"></i>
                </span>
              </router-link>
           <router-link :to="{ name: 'Editpayrolldriver', params: data.item }">
               <span class="mr-3">
