...
export default {
  ...
  methods: {
    ...
    async getClientDetails() {
      ...
      this.ServicerequestsOpt = this.clientTemplete.addressTemplateData.ServicerequestsData
      console.log(this.clientTemplete)
    },
    async changeServicerequests(value) {
      console.log(value)
      const result = await ServicerequestsTemplete(value)
          ...
          address: [
            {
              ...
              Servicerequests: this.form.Servicerequests,
      ...
      >
        <b-col md="12">
          <b-button
            ...
            >Create Servicerequests</b-button
          >
        </b-col>
        <div class="mt-3">
          <b-table
            ...
            :small="small"
            :fixed="fixed"
            :fields="ServicerequestsColumns"
            :items="item"
            class="mt-3"
          >
            <template slot="actions" slot-scope="data">
              <span class="mr-2" @click="editServicerequests(data)">
                  <i class="fas fa-pencil-alt edit"></i>
                </span>
               <span @click="deleteServicerequests(data)">
              <i class="fa fa-times edit"></i>
            </span>
              <!-- <b-button size="sm" class="mr-2" variant="html5 icon" @click="deleteServicerequests(data)">
              <i class="fa fa-times"></i>
            </b-button>
            <b-button size="sm" class="mr-2" variant="facebook" @click="editServicerequests(data)">
            ...
            <template slot="row-details" slot-scope="row">
              <b-card class="role-details">
                <div>
                  <multiselect
                    :id="row.item._id"
                    :value="ServicerequestsMap[row.item._id]"
                    :options="allServicerequests"
                    label="Servicerequests"
                    track-by="Servicerequests"
                    :multiple="true"
                    @select="
                      (selectedOption, id) =>
                        addServicerequestsToRole(selectedOption, id, row)
                    "
                  ></multiselect>
                </div>
              </b-card>
          ...
          <b-modal id="roleModal" v-model="ServicerequestsModal">
            <div slot="modal-header">{{ header }}</div>
            <div>
              <b-input v-model="Servicerequests" />

              <b-input v-model="Servicerequests" class="mt-2" />
